...
def rand_permutation(data_A, data_B, n, R):
    ...
    for x in range(0, R):
        temp_A = data_A
        temp_B = data_B
        samples = [np.random.randint(1, 3) for i in iter(range(n))] #which samples to swap without repetitions
...
def main():
    ...
    if(name=="Shapiro-Wilk" or name=="Anderson-Darling" or name=="Kolmogorov-Smirnov"):
        ...
        if(float(output)>float(alpha)):
            print("The normal test is significant")
            # if(answer=='Y'):
            #     # two sided t-test
            ...
            print("The normal test is not significant")
            # if (answer == 'Y'):
    ...
    if(name=="t-test"):
        ...
        if (float(pval) <= float(alpha)):
            ...
            print("\nTest result is \033[1m not \033[0m' significant with p-value: {}".format(pval))
            return
    ...
    if(name=="Wilcoxon"):
        ...
        if (float(wilcoxon_results[1]) <= float(alpha)):
            ...
            print("\nTest result is \033[1m not \033[0m' significant with p-value: {}".format(wilcoxon_results[1]))
            return
    ...
    if(name=="McNemar"):
        ...
        if (float(mcnemar_results) <= float(alpha)):
            ...
            print("\nTest result is \033[1m not \033[0m' significant with p-value: {}".format(mcnemar_results))
            return
