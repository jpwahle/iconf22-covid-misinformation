...
def evaluate(prediction_dataloader, model, path_to_model, model_name, load = False):
	...
	clsf_report.to_csv(str('results/'+bert_model+'_cmu.csv'), index= True)
...
def train_bert(net, criterion, opti, lr, lr_scheduler, train_loader, val_loader, epochs, iters_to_accumulate, bert_model):
    ...
    if len(bert_model) > 1:
    	...
    	bert_model = bert_model[0]
    path_to_model='models/cmu_{}_lr_{}_val_loss_{}_ep_{}.pt'.format(bert_model, lr, round(best_loss, 5), best_ep)
...
def main():
	...
	path_to_model = 'models/covid-deberta_lr_2e-05_val_loss_0.5127_ep_3.pt'
	evaluate(test_dataloader, model, path_to_model, model_name, load = False)
