...
class SentencePairClassifier(nn.Module):
    ...
    def __init__(self, bert_model="bert-base-uncased", freeze_bert=False):
        ...
        self.cls_layer = nn.Linear(self.hidden_size, 16)
...
def get_data(train_df, test_df):
	X1, Y_train = train_df['text'], train_df['label']
	x1_test, y_test = test_df['text'], test_df['label']
	...
	x1_val, y_val = x1_val.tolist(), y_val.tolist()
	x1_test, y_test = x1_test.tolist(), y_test.tolist()
	return (x1_train, y_train), (x1_val, y_val), (x1_test, y_test)
...
def gen_random_samples(test_df):
	...
	x1_sample_list, y_sample_list = [], []
	for i in range(150):
		chosen_indices = random.sample(index_list, num_samples)
		x1_sample, y_sample = test_df['text'][chosen_indices], test_df['label'][chosen_indices]
		x1_sample, y_sample = x1_sample.tolist(), y_sample.tolist()
	...
	# sample_chosen.to_csv("resultscmu/significance/sample_chosen.csv", index=False)
	return x1_sample_list, y_sample_list


################ Tokenizer ####################
###############################################
def tokenize(model_name, text_data, tokenizer, MAX_LEN):
	print('Tokenizing')
	# add special tokens for BERT to work properly
	model_lst = ["covid-roberta", "covid-bart", "covid-longformer", "vinai/bertweet-base"]
	if model_name in model_lst:
		sentences = ["<s> " + text_data[i] + " </s>" for i in range(0,len(text_data))]
	else:
		sentences = ["[CLS] " + text_data[i] + " [SEP]" for i in range(0,len(text_data))]
...
def evaluate(prediction_dataloader, model, model_name, path_to_model, load = False):
	...
	print('DONE.')
	labels_list = ['irrelevant', 'true public health response', 'calling out or correction', 
	'ambiguous or hard to classify', 'sarcasm or satire', 'false fact or prevention', 'conspiracy', 
	'true prevention', 'politics', 'fake cure', 'commercial activity or promotion', 
	'panic buying', 'news', 'fake treatment', 'emergency', 'false public health response']

	# Code for result display
	print('CMU classification accuracy is')
	print(metrics.accuracy_score(true_labels, predictions)*100)
	print(classification_report(true_labels, predictions, target_names = labels_list))
	# Converting to csv
	...
	clsf_report = pd.DataFrame(classification_report(y_true = true_labels, y_pred = predictions, output_dict=True, target_names = labels_list))
	clsf_report.to_csv(str('resultscmu/'+bert_model+'_cmu.csv'), index= True)

def significance(sample_dataloader_list, model, model_name, models_path='resultscmu/models'):
	...
	for file in onlyfiles:
		if file.split('_lr')[0].split('cmu_')[1] == bert_model:
	...
	file_name = 'resultscmu/significance/' + bert_model + '_cmu.txt'
	with open(file_name, 'w') as infile:
		for score in f1_score_list:
			print(score, file=infile)
	print("Printed F1 for model", bert_model)

def prepare_sample_loader(batch_size, model_name, x1_sample_list, tokenizer, MAX_LEN, y_sample_list):
	sample_dataloader_list = []
	for i in range(len(x1_sample_list)):
		sample_inputs, sample_masks, sample_token_ids = tokenize(model_name, x1_sample_list[i], tokenizer, MAX_LEN)
...
def train_bert(net, criterion, opti, lr, lr_scheduler, train_loader, val_loader, epochs, iters_to_accumulate, bert_model):
    ...
    if len(bert_model) > 1:
    	...
    	bert_model = bert_model[0]
    path_to_model='resultscmu/models/cmu_{}_lr_{}_val_loss_{}_ep_{}.pt'.format(bert_model, lr, round(best_loss, 5), best_ep)
...
def main():
	...
	print('Loading CMU_Covid dataset')
	train_df = pd.read_csv('datasets/CMU_MisCov_train.csv')
	print(train_df.columns)

	# Test set
	test_df = pd.read_csv('datasets/CMU_MisCov_test.csv')
	print(test_df.columns)

	(x1_train, y_train), (x1_val, y_val), (x1_test, y_test) = get_data(train_df, test_df)
	x1_sample_list, y_sample_list = gen_random_samples(test_df)
	...
	for model_name in ips:
		...
		train_inputs, train_masks, train_token_ids = tokenize(model_name, x1_train, tokenizer, MAX_LEN)
		val_inputs, val_masks, val_token_ids = tokenize(model_name, x1_val, tokenizer, MAX_LEN)
		test_inputs, test_masks, test_token_ids = tokenize(model_name, x1_test, tokenizer, MAX_LEN)
		...
		sample_dataloader_list = prepare_sample_loader(batch_size, model_name, x1_sample_list, tokenizer, MAX_LEN, y_sample_list)
