...
class SentencePairClassifier(nn.Module):
    ...
    def forward(self, input_ids, attn_masks, token_type_ids):
        ...
        special_models = ['roberta-base', 'distilbert-base-uncased', 'covid-roberta', 'facebook/bart-base',
        					'covid-bart', 'covid-longformer', "vinai/bertweet-base", "covid-distilbert"]
...
def gen_random_samples(test_df):
	...
	# sample_chosen.to_csv("resultscovfn19/significance/sample_chosen.csv", index=False)
...
def tokenize(model_name, premise_data, hypothesis_data, tokenizer, MAX_LEN):
	...
	if model_name in model_lst:
		sentences = ["<s> " + str(premise_data[i]) + " </s>" + str(hypothesis_data[i]) + "</s>" for i in range(0,len(premise_data))]
	else:
		sentences = ["[CLS] " + str(premise_data[i]) + " [SEP]" + str(hypothesis_data[i]) + "[SEP]" for i in range(0,len(premise_data))]
...
def evaluate(prediction_dataloader, model, model_name, path_to_model, load = False):
	...
	print('Covid19FN classification accuracy is')
	...
	clsf_report.to_csv(str('resultscovfn19/'+bert_model+'_fn19.csv'), index= True)


def significance(sample_dataloader_list, model, model_name, models_path='resultscovfn19/models'):
	...
	file_name = 'resultscovfn19/significance/' + bert_model + '_covid19fn.txt'
...
def train_bert(net, criterion, opti, lr, lr_scheduler, train_loader, val_loader, epochs, iters_to_accumulate, bert_model):
    ...
    if len(bert_model) > 1:
    	...
    	bert_model = bert_model[0]
    path_to_model='resultscovfn19/models/{}_lr_{}_val_loss_{}_ep_{}.pt'.format(bert_model, lr, round(best_loss, 5), best_ep)
...
def main():
	...
	print('Loading Covid19FN dataset')
	train_df = pd.read_csv('datasets/Train_CovidFN_News.csv')
	print(train_df.columns)

	# Test set
	test_df = pd.read_csv('datasets/Test_CovidFN_News.csv')
	...
	for model_name in ips:
		...
		#batch_size = 4
		big_models = ['digitalepidemiologylab/covid-twitter-bert-v2', 'covid-twitter-bert'
						'vinai/bertweet-base', 'manueltonneau/clinicalcovid-bert-base-cased',
						'manueltonneau/biocovid-bert-large-cased', 'allenai/longformer-base-4096']
		...
		# Significance Test
		models_path = 'resultscovfn19/models'
